...
Then $u(x,t) = \frac{1}{(4\pi t)^{n/2}}\int\limits_\R e^{\frac{-\absval{x-y}^2}{4t}} g(y) dy$. Let $\ep >0$. There is an $h \in \R^n$ with compact support such that $\absval{g}\geq \absval{h}$, $\absval{\int\limits_{\R^n} g - h} < \ep/2$ and $\absval{g-h} < \ep/(2A)$ where $A$ is the volume of a ball, call it $B$, that $h$ vanishes outside of. %Approximate g from below by a function with compact support. Make the outside \ep/2 and take t large enough that the inside is \ep/2. Win. Also, that's not the right amount of power for this...
