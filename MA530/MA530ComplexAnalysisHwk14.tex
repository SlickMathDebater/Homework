...
%Finish out the arithmetic, and you should get your desired result.

\shunt

{\bf Problem 5:}

Let $f \in \scrO(\Om)$, where $\Om$ is a symmetric domain (with respect to $\R$), and $\R \cap \Om \neq \emptyset$. Moreover, let $f(\R \cap \Om) \subset \R$. Then the function $g(z)=f(Re(z))$ is a holomorphic function. %Is that true?
