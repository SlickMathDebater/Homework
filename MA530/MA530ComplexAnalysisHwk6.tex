...
Now, if $k \in \Z^-$, then find $h$ with $h^{-k} = f$. Next, define $g = 1/h$. Then we have that $g^k = \frac{1}{h}^k =e^{\ln(1/h)k} = e^{-ln(h)k}= h^{-k} = f$, which yields our result. %Why do we get the middle step?
...
By the earlier homework, the sums of log-convex functions are log-convex. By a trivial induction argument, all finite sums of log-convex functions are log-convex. Moreover, $\ln(\absval{a_n}^2 s^{2n})$ is convex; %Prove it.
...
Now, the limit of a sequence of log-convex functions is log-convex: %Proof
