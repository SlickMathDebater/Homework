...
Because $f_n \to f$ in measure, we have that there is an $N \in \N$ such that for all $n \geq N$, %Something...

%I need to get $f_n \to f$ almost everywhere on a set except of measure $\de_n$. But that doesn't make sense...
...
{\bf Problem 3:} %Use the definitions, asshole.
...
%We get that tf < tf_n for all n and so by taking limits we get the result.
...
{\bf Problem 5:} %This is a little wrong. You need to use the \ep 2^-n trick to interface between the countable and the finite.
...
%Take a finite chunk of them that fails to cover some. We can take another such collection of "size" \de/4, and a finite chunk of them that fails to cover some. We should be able to repeat this process infinitely, and hit everything. Moreover, we can win with the above trick.
