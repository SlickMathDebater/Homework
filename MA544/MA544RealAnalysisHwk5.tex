...
{\bf Problem 2:} %A bit of incompleteness
...
\tab Consider the set $A = \{\al \in [x,y]: f(\al) > f(y)\}$. This set has a supremum, as it's nonempty. Define $\al =\sup(A)$. There is a $\de>0$ such that if $t \in [\al, \al+\de]$, then $f(t) < f(y)$: (Is pretty trivial). Moreover, $f(\al) = f(y)$: (Proof is clear, follows from continuity). So for any sequence $(t_n)$ decreasing to $\al$, we have $\frac{f(t_n) - f(\al)}{t_n - \al}$ negative. This means that $D^+(\al) \leq 0$. This contradicts our assumption on $D^+$. %Incompleteness here.
...
\tab We also know that $f^+(b) - f^+(a) \leq P_a^b$; %I'm not quite sure why. But this is the last chip; I'm almost done.
